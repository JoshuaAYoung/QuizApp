  ...
  questions: [
    {
      ...
      options: ["Muggle", "Hogwarts", "Quidditch", "Voldemort"],
      answer: "Muggle",
      explanation:
    ...
    },
    {
      ...
      explanation:
        "Bill, Charlie, Percy, Fred, George, Ron and Ginny are the children of Arthur and Molly Weasley. While it's never explicitly written or said, Fred is assumed to be the older of the Weasley twins.",
    ...
    },
    {
      ...
      explanation:
        'In "The Deathly Hallows," the trio of protagonists first learn of the triangle symbol that represents the three Deathly Hallows in the book The "Tales of Beedle the Bard"',
    ...
    },
    {
      //7
      question:
        "In the first line of the first book of the Harry Potter series, we learn that Harry's aunt and uncle, the Dursleys, were proud to say that they were:",
      options: [
    ...
    },
    {
      //10
      question:
        'In the third book, "Harry Potter and the Prisoner of Azkaban," Harry runs away from his aunt and uncle\'s home after accidentally:',
      options: [
    ...
    },
    {
      ...
      explanation:
        "The Marauder's Map, a magical document that revealed all of Hogwarts, was created by Remus Lupin (Moony), Peter Pettigrew (Wormtail), Sirius Black (Padfoot), and James Potter (Prongs) while they were attending the school.",
    ...
    },
    {
      ...
      explanation:
        "Professor Albus Percival Wulfric Brian Dumbledore was an English half-blood wizard, who was the Defence Against the Dark Arts Professor, later the Transfiguration Professor, and later still Headmaster of Hogwarts School of Witchcraft and Wizardry.",
    ...
    },
    {
      //13
      question:
        "What are the three cores Garrick Ollivander uses in his wand-making?",
      options: [
      ...
      explanation:
        "Although wand cores vary widely, certain wandmakers prefer to use certain materials; for example, Garrick Ollivander discovered and pioneered the use of phoenix feathers, dragon heartstrings, and unicorn tail hairs, whereas his father used lesser substances such as Kelpie hair and Kneazle whiskers",
    ...
    },
    {
      ...
      explanation:
        "James and Lily Potter were murdered by Lord Voldemort when Harry was one years old. To avoid associating themselves with the wizarding world, the Dursleys claimed that Harry's parents died in a car crash.",
      image: "./images/harryparents.jpg",
      imageAlt:
        "Artist rendition of Harry Potter and his parents, Lily and James.",
    },
    {
      //15
      question:
        "What is the LAST line of Harry Potter and the Deathly Hallows?",
      options: [
        "Harry nodded.",
      ...
      explanation:
        "At the end of book 7, Harry Potter and the Deahtly Hallows, J.K. Rowling paints a picture of the wizarding world 19 years later as Harry Potter and friends watch their kids board the Hogwarts Express.",
      image: "./images/pottergrown.jpg",
      imageAlt:
        "Artist rendition of Harry, Ron, Hermoine and Ginny all grown with their children",
    },
  ],
  userRank: [
    {
    ...
    },
    {
      rank: "Squib",
      rankText:
        "Without reading any of the books, you probably watched the last two movies because the other ones didn't get very good ratings, right?",
    ...
    },
    {
      rank: "First-Year",
      rankText:
        "Hey you're getting there. I'd avoid getting into in-depth discussions about H.P. with strangers on the bus until you've read the books a few more times, though.",
    ...
    },
    {
      rank: "D.A Member",
      rankText:
        "Killing it. If you want to go a little deeper, you should try adding the audiobooks to your collection. I prefer Stephen Fry as a narrator, but Jim Dale also offers something to the series. Try both for 10 points to Gryffindor",
      image: "./images/da.jpg",
    ...
    },
    {
      rank: "Auror",
      rankText:
        "Riddikulus! Albus would be so proud. Also, remind me never to challenge you to a duel.",
...
function assignRank() {
  if (APP.score / STORE.questions.length <= 0.2) {
    APP.rankIndex = 0;
    return APP.rankIndex;
  } else if (APP.score / STORE.questions.length <= 0.4) {
    APP.rankIndex = 1;
    return APP.rankIndex;
  } else if (APP.score / STORE.questions.length <= 0.6) {
    APP.rankIndex = 2;
    return APP.rankIndex;
  } else if (APP.score / STORE.questions.length <= 0.8) {
    APP.rankIndex = 3;
    return APP.rankIndex;
        ...
        <form class="questionForm">
            <fieldset>
                    <legend class="questionText">${
                      STORE.questions[APP.currentQuestion - 1].question
                    }</legend>
                    <p>
                        <input type="radio" name="options" id="firstQuestion" value="${
                          storeOption[0]
                        }" required>
                        <label class="questionOption" for="firstQuestion">${
                          storeOption[0]
                        }</label>
                    </p>
                    <p>
                        <input type="radio" name="options" id="secondQuestion" value="${
                          storeOption[1]
                        }" required>
                        <label class="questionOption" for="secondQuestion">${
                          storeOption[1]
                        }</label>
                    </p>
                    <p>
                        <input type="radio" name="options" id="thirdQuestion" value="${
                          storeOption[2]
                        }" required>   
                        <label class="questionOption" for="thirdQuestion">${
                          storeOption[2]
                        }</label>
                    </p>
                    <p>
                        <input type="radio" name="options" id="fourthQuestion" value="${
                          storeOption[3]
                        }" required>   
                        <label class="questionOption" for="fourthQuestion">${
                          storeOption[3]
                        }</label>
                    </p>
...
function generateRight() {
  ...
  return `<section class="answerView" id="checkAnswer" aria-live="polite">
        ...
        <img src="${questionArray.image}" alt="${
    questionArray.imageAlt
  }" class="answerImage">
    </section>`;
}
...
function generateResults() {
  return `<section class="finalView" id="startOver" aria-live="polite">
        ...
        <p class="resultsText">
            You got  <span class="scoreResults">${
              APP.score
            }</span>  out of  <span class="scoreResults">${
    STORE.questions.length
  }</span>  right.
        </p>
        <p class="rankText">
            Current Rank: <span class="userRank">"${
              STORE.userRank[assignRank()].rank
            }"</span>
        </p>
        ...
        <img src="${STORE.userRank[assignRank()].image}" alt="${
    STORE.userRank[assignRank()].imageAlt
  }" class="userRatingImage">
    </section>`;
}
...
function handleSubmitAnswer() {
  $(".questionForm").on("submit", function () {
    ...
    if (
      STORE.questions[APP.currentQuestion - 1].answer ==
      $(this).find('input[name="options"]:checked').val()
    ) {
      APP.score++;
